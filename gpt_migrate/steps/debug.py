...
def debug_error(error_message, relevant_files, globals):
    """
    Debug the error encountered during the migration process.

    This function receives the error message, relevant files, and global configuration as inputs.
    It prompts the user to take actions for debugging based on the error message and provides options to move files, edit files, or create new files.
    Depending on the user's choices, it executes shell scripts for debugging or re-writes files based on the error message.

    Args:
        error_message (str): The error message encountered during migration.
        relevant_files (str): Relevant files affected by the error.
        globals (obj): Global configuration object.
    ...
    prompt = identify_action_template.format(
    ...
    actions = llm_run(
        prompt,
        waiting_message="Planning actions for debugging...",
        success_message="",
        globals=globals
    )
    ...
    if "EDIT_FILES" in action_list:
        ...
        prompt = identify_file_template.format(
            error_message=error_message[-min(MAX_ERROR_MESSAGE_CHARACTERS, len(error_message)):],
            target_directory_structure=build_directory_structure(globals.targetdir),
        ...
        file_names = llm_run(
            prompt,
            waiting_message="Identifying files to debug...",
            success_message="",
            globals=globals
        )
        ...
        for file_name in file_name_list:
            ...
            prompt = debug_file_template.format(
                error_message=error_message[-min(MAX_ERROR_MESSAGE_CHARACTERS, len(error_message)):],
                file_name=file_name,
            ...
            _, language, file_content = llm_write_file(
                prompt,
    ...
    if "CREATE_FILE" in action_list:
        ...
        prompt = create_file_template.format(
            error_message=error_message[-min(MAX_ERROR_MESSAGE_CHARACTERS, len(error_message)):],
            target_directory_structure=build_directory_structure(globals.targetdir),
            guidelines=globals.guidelines
        )
        ...
        new_file_name, language, file_content = llm_write_file(
            prompt,
            waiting_message="Creating a new file...",
            success_message="",
            globals=globals
        )
...
def debug_testfile(error_message, testfile, globals):
    """
    Debug the error encountered during the test file migration process.

    This function receives the error message, test file, and global configuration as inputs.
    It prompts the user to debug the test file based on the error message and re-writes the test file if necessary.

    Args:
        error_message (str): The error message encountered during test file migration.
        testfile (str): The test file affected by the error.
        globals (obj): Global configuration object.
...
def require_human_intervention(error_message, relevant_files, globals):
    """
    Request human intervention for resolving the error encountered during migration.

    This function receives the error message, relevant files, and global configuration as inputs.
    It prompts the user to provide instructions for fixing the error and displays the instructions.
    It then exits with a message to resume the debugging process with the `--step test` flag.
    ...
    prompt = human_intervention_template.format(
        error_message=error_message[-min(MAX_ERROR_MESSAGE_CHARACTERS, len(error_message)):],
        relevant_files=relevant_files,
        target_directory_structure=build_directory_structure(globals.targetdir),
        guidelines=globals.guidelines
    ...
    instructions = llm_run(
        prompt,
        waiting_message="Writing instructions for how to proceed...",
        success_message="",
        globals=globals
    )

    typer.echo(typer.style("GPT-Migrate is having some trouble debugging your app and requires human intervention. Below are instructions for how to fix your application.", fg=typer.colors.BLUE))
    print(instructions)
    typer.echo(typer.style("Once the fix is implemented, you can pick up from the testing phase using the `--step test` flag.", fg=typer.colors.BLUE))
